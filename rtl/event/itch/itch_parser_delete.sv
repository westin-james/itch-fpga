        ...
        if (reset) begin
            ...
            if (data_valid) begin
                if (data_start) begin
                    ...
                    if (data_in != MSG_ORDER_DELETE) begin
                        parsing_message <= 1'b0;
                        parse_error <= 1'b1;
                    end
                    ...
                    if (data_last) begin
                        parsing_message <= 1'b0;
                        if (message_type == MSG_ORDER_DELETE
                        && captured_length == LEN_ORDER_DELETE
                        && byte_index == LEN_ORDER_DELETE - 1) begin
                            event_valid <= 1'b1;
                            event_data.event_type <= MSG_ORDER_DELETE;
                        end else begin
                            parse_error <= 1'b1;
                        end
                    end else begin
